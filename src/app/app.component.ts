    ...
    for (const colorName in palette) {
      ...
      if (typeof colorShades === "object") {
        for (const shade in colorShades) {
          ...
          if(colorName === "borderRadius") document.documentElement.style.setProperty(`--${colorName}-${shade}`, value);
          else document.documentElement.style.setProperty(`--color-${colorName}-${shade}`, value);
        }
      }
    }

    this._registry.addSvg("logo", "<?xml version=\"1.0\" encoding=\"iso-8859-1\"?>\n" +
